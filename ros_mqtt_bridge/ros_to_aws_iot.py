...
class ROSToAWSIoT(RospyArgsSetter, AWSIoTArgsSetter):
    ...
    def connect_mqtt(self):
        ...
        if "configure_credentials" in self.args.keys():
            self.__mqtt_client.configureCredentials(**self.args.configure_credentials)
        elif "configure_iam_credentials" in self.args.keys():
            self.__mqtt_client.configureIAMCredentials(**self.args.configure_iam_credentials)
        if "configure_last_will" in self.args.keys():
            self.__mqtt_client.configureLastWill(**self.args.configure_last_will)
